...
; Bind pointers after a successful local pattern scan. Mirrors Core_Initialize
; from after Scanner_ScanAllPatterns through Return $g_h_GWWindow.
Func Wine_FinishCoreInitialize($a_b_ChangeTitle = False)
	...
	EndIf
	If Not IsArray($g_ap_ScanResults) Then Return SetError(2, 0, 0)
	Local $l_p_Temp
    ;Core
	...
	; Salvage results live in $g_ap_ScanResults, not the injected ScanProc labels.
	Memory_SetValue('Environment', Ptr(Scanner_GetScanResult('Environment', $g_ap_ScanResults, 'Ptr')))
	;Core log
