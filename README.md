...
12. Live pos that does not close is a dead Engine drain, not an arrival. The GwAu3 Engine `fld [ebp+8]` site and the GWA2 `call eax` site both stayed at ticks=0 on wine-gw (`nearCallers=0`). After that, plant **one** WndProc trampoline. Wine `WinList` often has empty titles even when X11 shows **Guild Wars Reforged** — find hwnd via `FindWindowA(ArenaNet_Dx_Window_Class)`, title, `gw.exe` class, and a desktop walk by Gw PID (do not skip untitled windows). Then ticks climb, mem QueueCounter advances, and `pos` changes.
