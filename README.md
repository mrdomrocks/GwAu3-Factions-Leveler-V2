...
12. Live pos that does not close on the waypoint is a CommandMove miss, not an arrival. After a Queue-page stub rewrite, sync AutoIt `QueueCounter` to the in-memory slot Engine drains; log `enqueue ok=` plus queue head before/after. If pos stays frozen ~7s, refresh CommandMove (rewire / reset head / rewrite stubs on the existing Engine page — no second JMP) and hop via intermediate coords. Success is changing `pos` toward the waypoint, then `1/24`.
