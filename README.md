...
6. Already-inside Zen on Wine is **map/CurrentMapID 246 only**. Master Togo on outpost 213 is a town NPC and must **not** skip Enter Mission or start Escort Togo. Stale `MissionObjectiveArraySize` on outpost 213 must **not** skip Enter Mission. On 213, do **not** Travel/resign 213→213 when InstanceInfo flickers explorable — go straight to Enter Mission.
...
10. Mid-mission Start on **map/CurrentMapID 246** must log Escort and `Map_Move` without blocking the GUI. Skip `Cache_SkillBar` / Pathfinder / settle-wait on Wine when already in Zen 246. `Out()` also appends `Logs/leveler.log` next to the script. Togo is a **mission ally**, not a hench — detect him from agents (name/model/Rt) **after** 246. Do not resign while Togo or a clearly-alive player is present **in 246**. Outpost 213 Togo must not abort Enter Mission.
