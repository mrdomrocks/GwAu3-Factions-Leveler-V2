...
12. Live pos that does not close is a dead Engine drain, not an arrival. The GwAu3 Engine `fld [ebp+8]` site and the GWA2 `call eax` site are the **same iterator** on wine-gw (`007E9591` / `007E95FD`, `nearCallers=0`, ticks=0 even when 7ccd322's fresh JMP is left in). WndProc cannot plant: this Wine/Gw exposes **no Win32 hwnd** for `ArenaNet_Dx_Window_Class` / **Guild Wars Reforged** (only `IoLookupWnd` + IME; the game surface is X11). After Engine then GWA2 fail, plant **one** GwAu3 **Render** hook (`F6C401741C68` at hook+0x68, return +0xA) — or SkillTimer `call esi` if Render is missing. Then ticks climb, mem QueueCounter advances, `pos` changes, and escort reaches `waypoint 1/24`.
