...
12. Live pos that does not close is a dead Engine drain, not an arrival. Log `engine ticks=` and `jmp=` / `MainProc`. If ticks stay stuck or mem QueueCounter stays 0, rewrite stubs on the Queue page and **re-point the one Engine JMP** at the current MainProc (same site, no second JMP). Refresh must fire when pos is unchanged across walk logs (~5s), not only after a 7s stay timer. Success is changing `pos` toward the waypoint, then `1/24`.
