...
12. Live pos that does not close is a dead Engine drain, not an arrival. A trailing `_("EngineTicks/4")` reservation bumps `ASMCodeOffset` and writes MainProc 4 bytes past the JMP target. Heartbeat is `inc dword[EngineTicks]` at `page+ASMSize`. **87b468d** fixed alignment (`MainProcBytes=609CFF05…`) but ticks stayed 0. **a8d9f84** restored the leftover E9 and planted a fresh Engine JMP on a new page (`QueueBase=0557000C`, `007E9591 → 0558000C`) — still `ticks=0`, `nearCallers=0`. The GwAu3 Engine `fld [ebp+8]` site is **not** on the live path on this Wine/Gw build. After that fresh plant, restore `8B EC D9 45 08` again and plant **one** GWA2 `call eax; add esp,4` hook (not Engine fld). If that never ticks, one WndProc trampoline that calls `Move` and `ljmp OldWndProc`. Then ticks climb, mem QueueCounter advances, and `pos` changes.
