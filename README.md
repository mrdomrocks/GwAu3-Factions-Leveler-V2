...
12. Live pos that does not close is a dead Engine drain, not an arrival. The GwAu3 Engine `fld [ebp+8]` site and the GWA2 `call eax` site both stayed at ticks=0 on wine-gw (`nearCallers=0`). After that, plant **one** WndProc trampoline on the real DX game window — **not** Wine helpers. Prefer `FindWindowA(ArenaNet_Dx_Window_Class)` / title containing **Guild Wars** / **Reforged**, else the largest client among that Gw PID. Skip `IoLookupWnd`, Default IME, 1x1, and untitled stubs (9017cb9 hooked IoLookupWnd 954x525 and never ticked). Log `Wine hwnd: using … class=ArenaNet_Dx_Window_Class title=[Guild Wars Reforged]`. Then ticks climb, mem QueueCounter advances, `pos` changes, and escort reaches `waypoint 1/24`.
