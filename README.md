...
5. At the Zen outpost (213) only: Wine hides the overlay, dismisses covering UI, and clicks the Enter Mission pill via PostMessage/ControlClick plus a **frame-mapped** MouseClick (outer “Guild Wars Reforged” rect, xdotool Y=59 — not the DX child’s Y≈30). Do **not** 18-click when there is no Enter Mission button (already inside, or a missed sweep <15s ago). Latch when map/CurrentMapID is **246** for >1s, **or** Togo+Vhang are both living party allies. Do not latch on `{ENTER}` + Type/IsWaitingForMission flicker while still on 213 with only the town Togo NPC.
6. Already-inside Zen on Wine is **map/CurrentMapID 246** or **Master Togo + Headmaster Vhang as party allies together**. Map IDs may stay 213 while the instance UI is up (mission objective, no Enter Mission button). Master Togo on outpost 213 **alone** is a town NPC and must **not** skip Enter Mission or start Escort Togo. Stale `MissionObjectiveArraySize` on outpost 213 must **not** skip Enter Mission. On 213 with no Vhang, do **not** Travel/resign 213→213 when InstanceInfo flickers explorable — reveal the party/mission UI, then click Enter Mission.
