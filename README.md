...
12. Live pos that does not close is a dead Engine drain, not an arrival. Engine `007E9591` and GWA2 `007E95FD` are the same dead iterator. WndProc cannot plant (no Win32 DX hwnd). Stock Render `F6C401741C68-0x68` on this Wine/Gw is mid-`add esp,4` (`C4 04 83 3D 9C`, ticks=0) — skip it. After GWA2, plant **one** SkillTimer `call esi` (`FFD68B4DF0`); if that never ticks, SceneContext then UIMessage. Log `planted one skilltimer JMP` and rising ticks / mem QC / moving `pos` / `waypoint 1/24`.
