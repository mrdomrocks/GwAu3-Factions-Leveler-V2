...
5. At the Zen outpost (213) only: Wine hides the On-Top leveler, then clicks the **top-bar Enter Mission pill** in **client coordinates** (PostMessage / ControlClick) on `ArenaNet_Dx_Window_Class`. Do not trust `ClientToScreen` — on wine-gw it mapped `636,22` → screen `636,52`, **above** the Gw window at Y=59. Screen clicks must sit inside `WinGetPos`. Success is explorable **246**, one enter line, not a 213 click-spam.
