...
12. Live pos that does not close is a dead Engine drain, not an arrival. Engine `007E9591` and GWA2 `007E95FD` are the same dead iterator. WndProc cannot plant (no Win32 DX hwnd). Stock Render `-0x68` is mid-instruction — skip it. After GWA2, plant **one** SkillTimer `call esi`. Leftover SkillTimer `E9` (tail `8B D8 8B 47 08`) is restored to `FF D6 8B 4D F0` on Start so a retest does not need a manual restore. 195ad24: ticks rose but `drainSeen` stayed 0 (queue page WPM invisible to SkillTimer). Drain now consumes a **PendingMove** flag+xyz on the EngineTicks page, then still `call ebx`/`retn` for the queue. Log leftover restore, `pending=1`, `drainSeen` rising, mem QC advancing, moving `pos`, `waypoint 1/24`.
