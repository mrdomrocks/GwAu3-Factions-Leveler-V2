...
If Wine_IsWine() Then
	...
	Out("Start calls Core_Initialize immediately. The first step installs a one-JMP queue (QueueBase + CommandMove). At Zen outpost 213 invite henches [2,3,1,8,5] then Enter Mission. Already-inside Zen is map/CurrentMapID 246 or Togo+Vhang together (map may still read 213). Togo NPC alone does not skip Enter.")
...
Func Leveler_AttachToGw()
	...
	$g_b_WineEngineProven = False
	$g_i_WineRelocateN = 0
	$g_h_WineRelocateAt = 0
	$g_s_WineDrainKind = "none"
	$g_s_WineAsmExit = "engine"
	$g_p_WineDeadHook = 0
	$g_s_WineHookSaved = ""
	$g_p_WineHookSaved = 0
	$g_p_WineWndProcOld = 0
