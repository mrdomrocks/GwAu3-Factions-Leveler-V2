...
Func Leveler_Step_ExtendInventory()
	...
	If Leveler_HasExtendedBags() Then
		...
		Return True
	EndIf
	; Scan backpack for a small bag and EquipBag it. Never buy while one is already there.
	...
	If Leveler_CountLooseBags($MODEL_BAG) > 0 Or Leveler_CountLooseBags($MODEL_BELT_POUCH) > 0 Then
		Out("[Step] Backpack already has a small bag; not buying more. Retrying EquipBag into bag slots.")
		Return False
	EndIf
	If Not Leveler_Travel($MAP_SHING_JEA) Then Return False
	Leveler_SetPacifist()
