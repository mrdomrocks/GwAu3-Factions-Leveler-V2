...
;
; Wine Start still calls Core_Initialize(pid) first (same as Fedora/desktop).
; A 4/78 local scan can return hwnd + charname with AgentBase=0 and QueueBase=0.
; After that, salvage is a read-only 4KB .text scan of the five criticals, then
; one Engine JMP (never Assembler_ModifyMemory / five detours). Start only
; enables the bot loop when AgentBase, MyID, Engine, Move, BasePointer, and
; QueueBase are live user pointers and the Engine site starts with E9.
...
Func Wine_ResetScannerState()
	...
	If IsDeclared("g_p_AgentBase") Then $g_p_AgentBase = 0
	If IsDeclared("g_p_BasePointer") Then $g_p_BasePointer = 0
	If IsDeclared("g_i_MyID") Then $g_i_MyID = 0
...
; Map_Move / Ui_Dialog / Core_SendPacket Core_Enqueue into $g_p_QueueBase.
; That buffer is only drained if Engine starts with E9 to MainProc.
Func Wine_EngineJmpLive()
	Local $pMain = 0
	If IsDeclared("g_amx2_Labels") Then $pMain = Memory_GetValue('MainStart')
	If Not Wine_IsUserPtr($pMain) Then Return False
	Local $sFive = Wine_CompactHex(Wine_ReadBytesHex($pMain, 5))
	Return StringLeft($sFive, 2) = "E9"
EndFunc

Func Wine_CoreReadyMiss()
	Local $sMiss = ""
	If Not Wine_IsUserPtr($g_p_AgentBase) Then $sMiss &= " AgentBase"
	If Not Wine_IsUserPtr($g_p_BasePointer) Then $sMiss &= " BasePointer"
	If Not Wine_IsUserPtr($g_i_MyID) Then $sMiss &= " MyID"
	If Not Wine_IsUserPtr(Memory_GetValue('Move')) Then $sMiss &= " Move"
	If Not Wine_IsUserPtr(Memory_GetValue('MainStart')) Then $sMiss &= " Engine"
	If Not Wine_IsUserPtr($g_p_QueueBase) Then $sMiss &= " QueueBase"
	If Wine_IsUserPtr(Memory_GetValue('MainStart')) And Not Wine_EngineJmpLive() Then $sMiss &= " EngineJMP"
	If $sMiss = "" Then Return ""
	Return StringTrimLeft($sMiss, 1)
EndFunc

Func Wine_CoreReady()
	If Not Wine_IsWine() Then Return True
	If Wine_CoreReadyMiss() <> "" Then Return False
	Return True
EndFunc

Func Wine_LogCorePointers($a_s_When)
	Local $pMove = Memory_GetValue('Move')
	Local $pMain = Memory_GetValue('MainStart')
	Local $sJmp = ""
	If Wine_IsUserPtr($pMain) Then $sJmp = Wine_ReadBytesHex($pMain, 5)
	Out("Wine pointers (" & $a_s_When & "): AgentBase=" & Wine_HexPtr($g_p_AgentBase) & _
			" BasePointer=" & Wine_HexPtr($g_p_BasePointer) & " MyID=" & Wine_HexPtr($g_i_MyID) & _
			" Move=" & Wine_HexPtr($pMove) & " MainStart=" & Wine_HexPtr($pMain) & _
			" QueueBase=" & Wine_HexPtr($g_p_QueueBase) & " site5=" & $sJmp)
	Local $sMiss = Wine_CoreReadyMiss()
	If $sMiss <> "" Then Out("Wine criticals missing: " & $sMiss)
EndFunc
...
; After Core_Initialize left zeros: 4KB bind of the five criticals, then one
; Engine JMP if the queue is still dead. Never Assembler_ModifyMemory.
Func Wine_SalvageAfterCoreInitialize()
	Out("Wine salvage: Core_Initialize left dead criticals; 4KB bind then Engine drain if needed")
	If Not Wine_EnsureGwOpen($g_i_GWProcessId) Then
		Out("Wine salvage: no process handle")
		Return False
	EndIf
	If $g_h_GWWindow = 0 Or $g_h_GWWindow = "" Then
		$g_h_GWWindow = Wine_FindGwHwnd($g_i_GWProcessId)
		If $g_h_GWWindow = 0 Then $g_h_GWWindow = Scanner_GetHwnd($g_i_GWProcessId)
	EndIf
	Wine_ProbeGwMemory()
	If Not Wine_TextFirst8Live() Then
		Out("Wine salvage: .text first8 is empty or unreadable; refusing 4KB RPM")
		Return False
	EndIf

	Local $bEngineWasLive = Wine_EngineJmpLive()
	Local $pOldMove = Memory_GetValue('Move')
	Local $pOldQueue = $g_p_QueueBase

	If Not Wine_RegisterCriticalPatterns() Then Return False
	...
	If Not Wine_HasCriticalPatterns($aResults) Then
		Out("Wine salvage: 4KB scan missed AgentBase/MyID/Engine/Move/BasePointer")
		Return False
	EndIf
	...
	If Wine_CoreReady() Then
		If $bEngineWasLive And Not Wine_IsUserPtr($pOldMove) Then
			Out("ATTACH FAILED: Engine JMP already planted while Move was 0, so CommandMove is encoded against null.")
			Out("Old Move=" & Wine_HexPtr($pOldMove) & " old QueueBase=" & Wine_HexPtr($pOldQueue) & _
					". Not planting a second JMP over the existing E9.")
			Return False
		EndIf
		Out("Wine salvage: criticals live without a new JMP (reusing Core_Initialize queue/hook)")
		Return True
	EndIf

	If $bEngineWasLive Then
		Out("ATTACH FAILED: Engine JMP already present but QueueBase/Move were not usable.")
		Out("Old Move=" & Wine_HexPtr($pOldMove) & " old QueueBase=" & Wine_HexPtr($pOldQueue) & _
				". Not planting a second JMP over the existing E9.")
		Return False
	EndIf
...
Func Wine_BindReadOnly($aResults)
	...
	Local $pMoveSite = Scanner_GetScanResult('Move', $aResults, 'Func')
	Local $pEngine = Scanner_GetScanResult('Engine', $aResults, 'Hook')
	Out("Wine sites: BasePointer=" & Wine_HexPtr($pBaseSite) & " AgentBase=" & Wine_HexPtr($pAgentSite) & _
			" MyID=" & Wine_HexPtr($pMySite) & " Move=" & Wine_HexPtr($pMoveSite) & " Engine=" & Wine_HexPtr($pEngine))
	...
	Local $pMove = Wine_ResolveFuncStart($pMoveSite)
	Out("Wine deref: BasePointer=" & Wine_HexPtr($g_p_BasePointer) & " AgentBase=" & Wine_HexPtr($g_p_AgentBase) & _
			" MyIDPtr=" & Wine_HexPtr($g_i_MyID) & " Move=" & Wine_HexPtr($pMove))
	...
	If $g_p_BasePointer = 0 Or $g_p_AgentBase = 0 Then
		...
		Return False
	EndIf
	If Not Wine_IsUserPtr($pMove) Then
		Out("Wine attach: Move func did not resolve to a user pointer.")
		Return False
	EndIf
	If Not Wine_IsUserPtr($g_i_MyID) Then
		Out("Wine attach: MyID ptr did not look like a live user pointer.")
		Return False
	EndIf

	Wine_ReplaceValue('BasePointer', Ptr($g_p_BasePointer))
	Wine_ReplaceValue('AgentBase', Ptr($g_p_AgentBase))
	Wine_ReplaceValue('MaxAgents', Ptr($g_i_MaxAgents))
	Wine_ReplaceValue('MyID', Ptr($g_i_MyID))
	...
	If $pHook <> 0 Then
		...
		Out("Wine bind: MainStart=" & Wine_HexPtr($pHook) & " site5=" & Wine_ReadBytesHex($pHook, 5))
	EndIf
	Return True
EndFunc
...
; 5 bytes at the candidate match the MainExit epilogue (8B EC D9 45 08) or an
; existing E9. Try scan result, result-1, result+1 (Wine 4KB scan has no ASM P-1).
Func Wine_FindEngineHookSite($a_p_Engine)
	...
	For $i = 0 To 2
		...
		If StringLeft($sFiveC, 2) = "E9" Then Return $pCand
	...
	Next
	If $pPatOnly <> 0 Then
		Out("Wine inject abort: Engine pattern matched at " & Wine_HexPtr($pPatOnly) & " but site 5 bytes are " & $sPatFive & " (expected 8B EC D9 45 08 or E9)")
		Return 0
	EndIf
...
; One RWX page for queue + Move/PacketSend/Dialog stubs, then exactly one
; Engine JMP. No Assembler_ModifyMemory. No five-JMP fallback.
Func Wine_TryMinimalEngineHook($a_ap_FirstResults)
	...
	Out("Wine inject targets: Move=" & Wine_HexPtr($pMove) & " PacketSend=" & Wine_HexPtr($pPacketSend) & _
			" PacketLocation=" & Wine_HexPtr($pPacketLoc) & " Dialog=" & Wine_HexPtr($pDialog) & " Interact=" & Wine_HexPtr($pInteract))
	...
	If StringLeft(Wine_CompactHex($sBefore), 2) = "E9" Then
		Return Wine_InjectAbort("Engine site already starts with E9 at " & Wine_HexPtr($pHook) & "; QueueBase was not live")
	EndIf
	...
	Out("Wine inject: QueueBase=" & Wine_HexPtr($pQueue) & " MainProc=" & Wine_HexPtr($pMain))
	...
	Out("Wine: single Engine JMP planted. QueueBase=" & Wine_HexPtr($g_p_QueueBase) & " MainStart=" & Wine_HexPtr($pHook) & " MainProc=" & Wine_HexPtr($pMain))
